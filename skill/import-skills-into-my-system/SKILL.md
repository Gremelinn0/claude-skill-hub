...
---
## Skill overlap — delegate the fine-grained merge

When several skills overlap, this skill keeps ownership of the **global architecture decision** but may delegate the detailed skill-to-skill analysis to [Skill Cleaner](../skill-cleaner/SKILL.md) when available.

Use Skill Cleaner to classify the overlap as:
...
Then feed that result back into the `DECISION_MATRIX`.

Important:

- local/user/project-specific rules remain higher priority than generic imported methodology unless explicitly superseded;
- Skill Cleaner does **not** decide the whole target architecture;
- this skill does **not** reimplement Skill Cleaner's merge/dedup logic.

This keeps the responsibilities separate:

```text
import-skills-into-my-system
→ architecture / owners / migration
→ skill overlap detected
→ skill-cleaner
→ duplicate / complement / specialization / alternative / conflict
→ DECISION_MATRIX
```
